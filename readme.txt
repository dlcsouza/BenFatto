...
To install the same http-server used during the implementation of this project, use the command "npm install http-server -g" and to execute it use the command "http-server ProjectFolder". The default port is 8080
...
FrontEnd description:
    ...
    Log list:
        this page lists in a table the logs returned from the database, according to the filter. The table has a pagination feature as well as the number of records to be displayed.
        When clicking on "View details", the system redirects the user to the edit / delete page of the selected record

    Upload log Files
        in this page, the user informs one or more log files to be written in the database.
        The files must respect the pattern shown in the image example that is located inside the PDF file of the technical challenge Ben Fatto.
